...
import com.zc.mall.core.common.web.annotation.OperatorSessionAnnotation;
...
public class OperatorSessionInterceptor extends HandlerInterceptorAdapter {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        if (handler instanceof HandlerMethod) {
            OperatorSessionAnnotation operatorSessionAnnotation = findAnnotation((HandlerMethod) handler, OperatorSessionAnnotation.class);
            //没有声明需要权限,或者声明不验证权限
            if (operatorSessionAnnotation == null) {
                return true;
            }
            ...
            Operator operator = operatorDao.find(sessionValue);
            if (operator == null) {
                throw new BusinessException("您还未登录");
            }
            if (operator.getState() != BaseConstant.INFO_STATE_YES) {
